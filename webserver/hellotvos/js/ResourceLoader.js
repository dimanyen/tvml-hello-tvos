...
ResourceLoader.prototype.httpRequest = function(url, callback)
{
    var xmlhttp = new XMLHttpRequest() , method = 'GET' , url = url;
    ...
    if (xmlhttp.readyState == 4) {
      ...
      if (status == 200) {
        ...
        var errorDoc = createAlert("Http request Error \n" + "Error loader:" + status);
...
ResourceLoader.prototype.loadResourceWithArg = function(resource, callback, argument) {
    var self = this;
    evaluateScripts([resource], function(success) {
        if (success) {
            var resource = Template.call(self,argument);
